...
def _serialize_state_locked():
    download_output = STEPS["download"]["local_output"] + STEPS["post_processing"]["local_output"]
    sync_output = STEPS["download"]["remote_output"] + STEPS["post_processing"]["remote_output"]
    error = WORKFLOW_STATE["error"] or STEPS["download"]["error"] or STEPS["post_processing"]["error"]
    return {
...
def monitoring_start_postprocessing():
    ...
    thread = threading.Thread(
        ...
        args=(account, configuration_data, memory, cpus, wall_time, slurm_account),
        daemon=True,
    )
    thread.start()
    return jsonify({"ok": True, **snapshot})
